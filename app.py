...
if mode == "Static (Upload File)":
    ...
    if train_file and new_file:
        ...
        graph_type = st.selectbox("Select Graph Type", ["Box Plot", "Bar Chart", "Scatter Plot", "Histogram"])
        ...
        if parameter in train_data.columns and parameter in new_data.columns:
            ...
            if graph_type == "Box Plot":
                ...
                fig = px.histogram(comparison_df, x=parameter, color="Dataset", barmode="overlay",
                                   title=f"Histogram Comparison of {parameter}",
                                   template="plotly_dark", marginal="box", nbins=30)

            fig.update_layout(
                xaxis_title=f"{parameter} Values",
                yaxis_title="Frequency" if graph_type == "Histogram" else f"{parameter} Values",
    ...
    if patients_data:
        ...
        if predict_button:
            ...
            for index, row in df.iterrows():
                ...
                risk = row['Risk (%)']
                st.markdown(f"🧍‍♂ {name}:** {risk:.2f}% risk")
                if risk < 30:
                    st.success("✅ Maintain current lifestyle, but continue regular health check-ups.")
                elif 30 <= risk <= 70:
                    st.warning("⚠ Risk is moderate. Consider reviewing diet, weight, and activity levels.")
                else:
